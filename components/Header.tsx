...
export const Header = memo(({ user, unreadCount, onOpenNotifs, onOpenProfile }: HeaderProps) => {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-6 py-3 pt-safe flex justify-between items-center bg-[#020617]/80 backdrop-blur-xl border-b border-white/5 transition-all duration-300" style={{ paddingTop: 'max(12px, env(safe-area-inset-top))' }}>
      <div className="flex items-center gap-3">
        ...
        {/* App Name */}
        <h1 className="text-3xl font-black font-outfit tracking-tighter bg-gradient-to-r from-white via-teal-100 to-teal-400 bg-clip-text text-transparent drop-shadow-sm">
          Flow
        </h1>
      </div>
